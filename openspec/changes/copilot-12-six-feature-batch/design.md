...
### D5: 頁籤拖曳排序 — HTML5 Native DnD

**選定方案**：HTML5 原生 drag-and-drop API，零外部依賴。

**替代方案**：`@dnd-kit/core` + `@dnd-kit/sortable` (~25KB gzipped)。
**取捨**：只需水平 1D 排序，native DnD 足夠。Store 已有 `reorderTabs()` 方法。如未來需要觸控裝置支援，可升級到 dnd-kit。

**視覺回饋**：
- 被拖曳 tab: `opacity-30`
- 目標 tab: 左/右邊顯示 2px accent 色指示線
- 透過 `e.clientX` 與 target bounding rect 中點比較判斷指示線位置

**防護措施**：拖曳中禁止 click/close/popover 操作，`onDragStart` 時關閉已開啟的 ConversationPopover。
...
- Zustand store 只存 `openspecPanelOpen: boolean`
