...
- `llmLanguage`: `string | null`（預設 `null`）— LLM 回應語言，`null` 表示跟隨 UI language
...
- **THEN** 系統回傳包含所有預設值的完整設定物件，llmLanguage 為 null，pushNotifications 為 undefined

#### Scenario: 讀取已存在的設定

- **WHEN** settings.json 存在且包含 `{ "theme": "light", "language": "zh-TW", "llmLanguage": "ja" }`
- **THEN** 系統回傳該物件，缺失欄位以預設值填充

#### Scenario: PATCH llmLanguage 設定

- **WHEN** 發送 `PATCH /api/settings` body 為 `{ "llmLanguage": "zh-TW" }`
- **THEN** llmLanguage 欄位更新為 `"zh-TW"`，其他設定不變

#### Scenario: 清除 llmLanguage 設定

- **WHEN** 發送 `PATCH /api/settings` body 為 `{ "llmLanguage": null }`
- **THEN** llmLanguage 欄位重設為 `null`（跟隨 UI 語言）
