...
當使用者在聊天輸入框中輸入 `@` 字元時，系統 SHALL 彈出檔案選擇下拉選單（AtFileMenu），顯示當前對話 cwd 下的檔案與目錄。

#### Scenario: 輸入 @ 觸發選單

- **WHEN** 使用者在輸入框中輸入 `@`（前方為字串開頭、空白或換行）
- **THEN** 系統 MUST 在輸入框上方顯示 AtFileMenu 下拉選單
- **AND** 選單 MUST 顯示對話 cwd 目錄下的目錄和檔案列表

#### Scenario: @ 後輸入文字過濾

- **WHEN** 使用者在 `@` 後繼續輸入文字（如 `@src`）
- **THEN** 選單 MUST 即時以 case-insensitive 子字串比對過濾顯示項目
